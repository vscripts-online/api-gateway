...
export function generateEncodedVerifyCode(_id: string, code: number) {
  ...
  const query = Buffer.from(new_id + signature, 'hex').toString('base64url') + code.toString(36)
  return query
}
...
export function decodeVerifyCode(query: string) {
  ...
  const query_hex = Buffer.from(code_excluded_query, 'base64url').toString('hex')
...
export function IsNotEqualWith(property: string, validationOptions?: ValidationOptions) {
