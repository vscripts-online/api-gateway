...
export class GoogleDrive extends BaseDrive {
  ...
  async get_drive(): Promise<drive_v3.Drive> {
    ...
    if (expire_time < ms('5 min')) {
      ...
      this.accountRepository.set_access_token(String(account._id), access_token, expiry_date)
  ...
  async get_storage_sizes(): Promise<Pick<IAccountSchema, "storage_size" | "available_size">> {
    ...
    const available_size = (storage_size - Number(data?.storageQuota?.usage)) || 0
  ...
  async get_file(file_part: IFilePartSchema) {
    const drive = await this.get_drive()
    const { data } = await drive.files.get({ fileId: file_part.id, alt: 'media' }, { responseType: 'stream' })
    return data
  }
