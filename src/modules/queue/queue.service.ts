...
export class QueueService {
  ...
  private async onModuleInit() {
    ...
    await new_file_upload_channel.assertQueue(RABBITMQ_CHANNELS.NEW_FILE_UPLOAD_QUEUE)
    this.channels.set(RABBITMQ_CHANNELS.NEW_FILE_UPLOAD_QUEUE, new_file_upload_channel)
  ...
  new_file(file: IFileSchema) {
    ...
    for (let i = 1; i <= Math.ceil(file.size / seperate_byte); i++) {
      const data: RABBITMQ_CHANNELS_DATAS['NEW_FILE_UPLOAD_QUEUE'] = { _id: file._id, offset, size: Math.min(seperate_byte, file.size - offset), name: file.name }
  ...
  send_email(_id: string, email: string, code: number) {
    ...
    const channel = this.channels.get(queue)
    const data: RABBITMQ_CHANNELS_DATAS['SEND_EMAİL_QUEUE'] = { _id, email, code }
    const body = JSON.stringify(data)
