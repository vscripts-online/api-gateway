...
import { calculateSeed, generateEncodedVerifyCode } from 'src/common/util';
...
export class ConsumerService {
  ...
  on_new_file() {
    ...
    channel.consume(queue, async data => {
      const { _id, offset, size, name } = JSON.parse(data.content.toString()) as RABBITMQ_CHANNELS_DATAS['NEW_FILE_UPLOAD_QUEUE']

      console.log('consumed', _id, offset, size, name)

      const account = await this.accountRepository.get_by_available_size_and_decrease(size)
      if (!account)
      ...
      const file_id = await this.storageService.upload(account, crypto.randomUUID(), file_stream)
      if (!file_id) {
        await this.accountRepository.increase_available_size(account._id, size)
        return channel.reject(data, true)
      }

      await this.fileRepository.create_file_part(_id, { owner: account._id, name, offset, size, id: file_id })
      channel.ack(data)
    })
  }
  ...
  on_send_email() {
    ...
    channel.consume(queue, async data => {
      const { _id, email, code } = JSON.parse(data.content.toString()) as RABBITMQ_CHANNELS_DATAS['SEND_EMAİL_QUEUE']

      const query = generateEncodedVerifyCode(_id, code)
      const generate_url = 'https://domain.com/change_password_from_forgot?query=' + query
