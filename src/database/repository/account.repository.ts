...
export class AccountRepository {
  ...
  async sync_size(_id: string, params: Pick<IAccountSchema, 'storage_size' | 'available_size'>): Promise<any> {
  ...
  async get_by_available_size_and_decrease(size: number) {
    return this.model.findOneAndUpdate({ available_size: { $gt: size } }, { $inc: { available_size: 0 - size } }).sort({ available_size: -1 })
  }

  async increase_available_size(_id: string, size: number) {
    return this.model.updateOne({ _id }, { $inc: { available_size: 0 - size } })
  }

  async set_access_token(_id: string, access_token: string, access_token_expiry_time: number) {
    return this.model.updateOne({ _id }, { $set: { access_token, access_token_expiry_time } })
  }
